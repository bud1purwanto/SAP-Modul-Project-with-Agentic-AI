...
FORM DOWNLOAD_BATCH_LIST_XLSX.
  ...
  LOOP AT LT_CFG.
    ...
    IF LT_CFG-FIELD_NAME = 'BATCH_NUMBER' OR
       LT_CFG-FIELD_NAME = 'TEXT_BARCODE'  OR
       LT_CFG-FIELD_NAME = 'TXT_BARCODE'   OR
       LT_CFG-FIELD_NAME = 'ROLL_NUMBER'   OR
       LT_CFG-FIELD_NAME = 'ROLL_NUM'      OR
       LT_CFG-FIELD_NAME = 'LOT_NUMBER'    OR
       LT_CFG-FIELD_NAME = 'LOT_NUM'       OR
       LT_CFG-FIELD_NAME = 'DO_NUMBER'     OR
       LT_CFG-FIELD_NAME = 'PO_NUMBER'     OR
       LT_CFG-FIELD_NAME = 'SO_NUMBER'     OR
       LT_CFG-FIELD_NAME = 'HU_NUMBER'     OR
       LT_CFG-FIELD_NAME = 'NO_PALET'      OR
       LT_CFG-FIELD_NAME = 'NO_PALET_TRIAS'.
  ...
  LOOP AT GT_DYN_ROLL.
    ...
    DO LV_COL_COUNT TIMES.
      ...
      ELSEIF LV_VALUE IS NOT INITIAL AND LV_VALUE(1) = '0' AND LV_VALUE CO '0123456789 '.
        SET PROPERTY OF LO_CELL 'NumberFormat' = '@'.
      ENDIF.
      IF LT_CFG-FIELD_NAME = 'BARCODE' AND LV_VALUE IS NOT INITIAL.
        CALL METHOD OF LO_SHEET 'Rows' = LO_HEADER_ROW EXPORTING #1 = LV_ROW.
        SET PROPERTY OF LO_HEADER_ROW 'RowHeight' = 35.
        SET PROPERTY OF LO_CELL 'Value' = ''.
        PERFORM INSERT_EXCEL_BARCODE_IMAGE USING LO_SHEET LO_CELL LV_VALUE.
      ELSE.
        SET PROPERTY OF LO_CELL 'Value' = LV_VALUE.
      ENDIF.
      SET PROPERTY OF LO_CELL 'HorizontalAlignment' = -4108.
...
FORM GET_COA_COL_VALUE USING P_FIELD_NAME TYPE ANY
  ...
  CASE LV_FLD.
    ...
    WHEN 'NO_PALET_TRIAS' OR 'PALET_TRIAS'.
      CLEAR LV_HPAL.
      PERFORM GET_HU_PALLET_TRIAS_INFO USING P_BATCH-VBELN P_BATCH-CHARG
                                    CHANGING LV_HPAL.
      P_RAW_VAL = LV_HPAL.
    ...
    WHEN 'TYPE' OR 'MAT_TYPE' OR 'ZZTYPE' OR 'ZZCODE'.
      ...
      IF LV_DESC_CD IS NOT INITIAL.
        SPLIT LV_DESC_CD AT '-' INTO P_RAW_VAL LV_ROLL_STR.
      ELSEIF LV_ZZCODE_VAL IS NOT INITIAL.
        SPLIT LV_ZZCODE_VAL AT '-' INTO P_RAW_VAL LV_ROLL_STR.
      ELSEIF P_BATCH-ZZTYPE IS NOT INITIAL.
        SPLIT P_BATCH-ZZTYPE AT '-' INTO P_RAW_VAL LV_ROLL_STR.
      ELSE.
    ...
    WHEN 'TREATMENT_IN' OR 'TREAT_IN'.
      IF GV_ATINN_INSIDE IS NOT INITIAL.
        ...
        IF SY-SUBRC = 0.
          PERFORM GET_CHAR_DESC USING GV_ATINN_INSIDE GS_CHAR_CACHE-ATWRT
                               CHANGING P_RAW_VAL.
          IF P_RAW_VAL IS INITIAL.
            P_RAW_VAL = GS_CHAR_CACHE-ATWRT.
          ENDIF.
        ENDIF.
      ENDIF.
    ...
    WHEN 'TREATMENT_OUT' OR 'TREAT_OUT'.
      IF GV_ATINN_OUTSIDE IS NOT INITIAL.
        ...
        IF SY-SUBRC = 0.
          PERFORM GET_CHAR_DESC USING GV_ATINN_OUTSIDE GS_CHAR_CACHE-ATWRT
                               CHANGING P_RAW_VAL.
          IF P_RAW_VAL IS INITIAL.
            P_RAW_VAL = GS_CHAR_CACHE-ATWRT.
          ENDIF.
...
FORM PREPARE_HU_CACHE USING P_VBELN TYPE LIPS-VBELN.
  ...
  " 4b. Bulk Select Pallet ID RTP from ZPALLETR & ZHMS using EXIDV
  DATA: BEGIN OF LT_PAL_EXIDV OCCURS 0,
          EXIDV TYPE ZPALLETR-EXIDV,
        END OF LT_PAL_EXIDV,
        BEGIN OF LT_ZPALLETR OCCURS 0,
          EXIDV  TYPE ZPALLETR-EXIDV,
          DOCNO  TYPE ZPALLETR-DOCNO,
          STATUS TYPE ZPALLETR-STATUS,
          PALID  TYPE ZPALLETR-PALID,
        END OF LT_ZPALLETR,
        BEGIN OF LT_ZHMS OCCURS 0,
          EXIDV TYPE ZHMS-EXIDV,
          PALID TYPE ZHMS-PALID,
        END OF LT_ZHMS.
  DATA: LV_PAD_EXIDV TYPE ZPALLETR-EXIDV.

  LOOP AT LT_PAL_VEKP.
    CLEAR LV_PAD_EXIDV.
    CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
      EXPORTING INPUT  = LT_PAL_VEKP-EXIDV
      IMPORTING OUTPUT = LV_PAD_EXIDV.
    LT_PAL_EXIDV-EXIDV = LV_PAD_EXIDV.
    APPEND LT_PAL_EXIDV.
  ENDLOOP.
  LOOP AT LT_BOX_VEKP WHERE UEVEL IS INITIAL.
    CLEAR LV_PAD_EXIDV.
    CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
      EXPORTING INPUT  = LT_BOX_VEKP-EXIDV
      IMPORTING OUTPUT = LV_PAD_EXIDV.
    LT_PAL_EXIDV-EXIDV = LV_PAD_EXIDV.
    APPEND LT_PAL_EXIDV.
  ENDLOOP.
  SORT LT_PAL_EXIDV BY EXIDV.
  DELETE ADJACENT DUPLICATES FROM LT_PAL_EXIDV.

  IF LT_PAL_EXIDV[] IS NOT INITIAL.
    SELECT EXIDV DOCNO STATUS PALID FROM ZPALLETR INTO TABLE LT_ZPALLETR
      FOR ALL ENTRIES IN LT_PAL_EXIDV
      WHERE EXIDV = LT_PAL_EXIDV-EXIDV
        AND STATUS IN ('C', 'E', 'P', 'L').
    IF LT_ZPALLETR[] IS NOT INITIAL.
      SORT LT_ZPALLETR BY EXIDV ASCENDING DOCNO DESCENDING.
      DELETE ADJACENT DUPLICATES FROM LT_ZPALLETR COMPARING EXIDV.
    ENDIF.

    SELECT EXIDV PALID FROM ZHMS INTO TABLE LT_ZHMS
      FOR ALL ENTRIES IN LT_PAL_EXIDV
      WHERE EXIDV = LT_PAL_EXIDV-EXIDV.
    IF LT_ZHMS[] IS NOT INITIAL.
      SORT LT_ZHMS BY EXIDV ASCENDING PALID DESCENDING.
      DELETE ADJACENT DUPLICATES FROM LT_ZHMS COMPARING EXIDV.
    ENDIF.
  ENDIF.
  ...
  DATA: LV_LOOKUP_EXIDV TYPE ZPALLETR-EXIDV.
  LOOP AT LT_VEPO.
    CLEAR GS_HU_CACHE.
    ...
    IF SY-SUBRC = 0.
      IF LT_BOX_VEKP-UEVEL IS NOT INITIAL.
        ...
        IF SY-SUBRC = 0.
          ...
          CALL FUNCTION 'CONVERSION_EXIT_ALPHA_INPUT'
            EXPORTING INPUT  = LT_PAL_VEKP-EXIDV
            IMPORTING OUTPUT = LV_LOOKUP_EXIDV.
          READ TABLE LT_ZPALLETR WITH KEY EXIDV = LV_LOOKUP_EXIDV BINARY SEARCH.
          IF SY-SUBRC = 0 AND LT_ZPALLETR-PALID IS NOT INITIAL.
            GS_HU_CACHE-NO_PALET_TRIAS = LT_ZPALLETR-PALID.
          ELSE.
            READ TABLE LT_ZHMS WITH KEY EXIDV = LV_LOOKUP_EXIDV BINARY SEARCH.
            IF SY-SUBRC = 0.
              GS_HU_CACHE-NO_PALET_TRIAS = LT_ZHMS-PALID.
            ENDIF.
          ENDIF.
        ENDIF.
      ELSE.
