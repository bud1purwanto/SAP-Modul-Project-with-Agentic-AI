...
FORM GET_DATA.
  ...
  LOOP AT IT_DATA.
    ...
    " Logika Pewarisan (Inheritance) - Batch Pertama sebagai Sampel
    IF IT_DATA-VBELN NE L_PREV_VBELN OR IT_DATA-MATNR NE L_PREV_MATNR.
      L_PREV_VBELN = IT_DATA-VBELN.
      L_PREV_MATNR = IT_DATA-MATNR.
      ...
      " Fix (permintaan user 15/07/2026): kondisi sebelumnya "IS INITIAL" - salah arah.
      " GET_TRACED_LOTS & GET_MIC WAJIB dipanggil justru saat IT_ZMAP TERISI (ada mapping
      " ZMAP_COA), karena GET_MIC melakukan LOOP AT IT_ZMAP - kalau IT_ZMAP kosong, loop itu
      " otomatis 0 iterasi (aman, tidak perlu di-skip manual). Kondisi lama menyebabkan MIC
      " tidak pernah terisi untuk material yang justru SUDAH dikonfigurasi di ZMAP_COA.
      IF IT_ZMAP[] IS NOT INITIAL.
        PERFORM GET_TRACED_LOTS.

        " Get JR Roll Number for the sample batch (From Bulk AUSP)
        L_TARGET_JR = L_JR_BASE_BATCH.
        IF L_TARGET_JR IS INITIAL.
          L_TARGET_JR = L_JR_CONV_BATCH.
        ENDIF.
    ...
    LOOP AT IT_MIC.
      ...
      IF IT_MIC-INS IS NOT INITIAL.
        ...
        IF IT_ALV-CMICMIT IS INITIAL.
          ...
          IF IT_ALV-CMICMIT IS INITIAL.
            SELECT SINGLE MITTELWERT INTO IT_MIC-MICMIT
              FROM QAMR
              WHERE PRUEFLOS = IT_MIC-INS
                AND MERKNR = IT_MIC-MERKNR.
