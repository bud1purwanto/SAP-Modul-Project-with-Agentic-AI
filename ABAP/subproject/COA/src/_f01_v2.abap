...
* dan berurutan). BKTXT menyimpan identitas transaksi / nomor roll, dan merupakan penghubung
* yang benar antara posting 101 dan 261 di dalam SATU production order.
...
FORM GET_VALID_ORDER_FROM_MSEG USING P_CHARG CHANGING P_AUFNR.
  ...
  IF SY-SUBRC = 0.
    ...
    IF SY-SUBRC = 0.
      ...
      " Fix A (permintaan user 18/07/2026): ambil BKTXT dokumen GR (101) ini, dipakai
      " GET_VALID_COMPONENT_FROM_MSEG untuk memilih komponen 261 yang BENAR-BENAR pasangan
      " transaksi/roll ini. Terbukti dari data DEV: order 100000072887 punya 8 posting 261
      " dengan 4 batch komponen berbeda, dibedakan HANYA oleh BKTXT (mis. "F GID 076 001"
      " s/d "004"). Logic lama ambil MBLNR terbesar -> salah roll tanpa gejala apa pun.
      " Referensi: ZQMR_TRACEABILITY (variabel VBKTXT).
      ...
      " level Base Film (mis. PALZTH00) selalu kosong. Ref: ZQMR_TRACEABILITY (VCOMBINE).
      CLEAR LV_COMBINE.
...
FORM GET_VALID_COMPONENT_FROM_MSEG USING P_AUFNR CHANGING P_CHARG.
  ...
  IF P_AUFNR IS INITIAL.
    RETURN.
  ENDIF.
  " Fix (18/07/2026): tambahkan movement type custom '901'/'902' selain '261'/'262',
  " mengikuti ZQMR_TRACEABILITY yang sudah terbukti benar untuk relasi 101 <-> 261.
  ...
  IF SY-SUBRC = 0.
    ...
    " Fix BKTXT (18/07/2026): buang komponen yang BUKAN milik transaksi/roll yang sama dengan
    " dokumen 101-nya. Satu production order bisa punya banyak roll/transaksi, dan BKTXT adalah
    " satu-satunya pembeda. Kalau BKTXT dokumen 101 kosong (data lama sebelum konvensi ini
    " dipakai), filter dilewati supaya kandidat tidak habis terhapus (fallback ke logic lama).
