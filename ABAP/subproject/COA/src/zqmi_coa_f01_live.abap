...
FORM F_PRINT .
  ...
  DATA: IT_ROLL TYPE ZQMSAP OCCURS 0 WITH HEADER LINE.
  DATA: LV_VALUE10(10) TYPE C.
...
*    it_QAMV-LTEXTEKZ = it_detail-TYPE.
  ...
  IF LT_ROLL_LOT[] IS NOT INITIAL.
    SELECT VBELN POSNR CHARG NTGEW
      INTO TABLE LT_LIPS
      FROM LIPS
      FOR ALL ENTRIES IN LT_ROLL_LOT
      WHERE VBELN = LT_ROLL_LOT-VBELN
        AND POSNR = LT_ROLL_LOT-POSNR
        AND CHARG = LT_ROLL_LOT-CHARG.
    SORT LT_LIPS BY VBELN POSNR CHARG.
  ENDIF.
  ...
  LOOP AT LT_ROLL_LOT.
    ...
    READ TABLE LT_LIPS WITH KEY
      VBELN = LT_ROLL_LOT-VBELN
      POSNR = LT_ROLL_LOT-POSNR
