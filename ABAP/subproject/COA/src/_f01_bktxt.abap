...
FORM GET_DATA.
  ...
  " TEMP DEBUG (permintaan user 17/07/2026): trace lot resolution per batch untuk investigasi
  " kasus "beda JR dalam 1 DO" (DO 0085006235). Tulis ke Application Log (SLG1), OBJECT
  " 'ZQMICOA' SUBOBJECT 'TRACE'. Murni tambahan, tidak mengubah logic bisnis/tracing/aggregasi
  " apapun. Aman dihapus kapan saja setelah investigasi selesai.
  DATA: LV_DBG_LOG_HANDLE TYPE BALLOGHNDL,
        LS_DBG_LOG        TYPE BAL_S_LOG,
        LS_DBG_MSG        TYPE BAL_S_MSG,
        LT_DBG_LOG_HANDLE TYPE BAL_T_LOGH.

  CLEAR : TEXT1, TEXT2,ROLL, R_PSTYV, IT_ZMAP_DELIV.
  REFRESH : R_PSTYV, IT_ZMAP_DELIV.

  " TEMP DEBUG: buat log header sekali per eksekusi report
  CLEAR LS_DBG_LOG.
  LS_DBG_LOG-OBJECT = 'ZQMICOA'.
  LS_DBG_LOG-SUBOBJECT = 'TRACE'.
  LS_DBG_LOG-EXTNUMBER = SY-UNAME.
  LS_DBG_LOG-ALDATE = SY-DATUM.
  LS_DBG_LOG-ALTIME = SY-UZEIT.
  CALL FUNCTION 'BAL_LOG_CREATE'
    EXPORTING
      I_S_LOG      = LS_DBG_LOG
    IMPORTING
      E_LOG_HANDLE = LV_DBG_LOG_HANDLE
    EXCEPTIONS
      OTHERS       = 9.
  ...
  LOOP AT IT_DATA.
    ...
    " TEMP DEBUG (permintaan user 17/07/2026): rekam hasil resolusi lot per batch
    CLEAR LS_DBG_MSG.
    LS_DBG_MSG-MSGTY = 'I'.
    LS_DBG_MSG-MSGID = '00'.
    LS_DBG_MSG-MSGNO = '398'.
    CONCATENATE 'CHARG=' IT_DATA-CHARG INTO LS_DBG_MSG-MSGV1.
    CONCATENATE 'SRC=' LOT_SR_CONV ' SRB=' LOT_SR_BASE INTO LS_DBG_MSG-MSGV2.
    CONCATENATE 'JRC=' LOT_JR_CONV ' JRB=' LOT_JR_BASE INTO LS_DBG_MSG-MSGV3.
    CONCATENATE 'ROLL=' IT_DATA-NOMSR INTO LS_DBG_MSG-MSGV4.
    CALL FUNCTION 'BAL_LOG_MSG_ADD'
      EXPORTING
        I_LOG_HANDLE = LV_DBG_LOG_HANDLE
        I_S_MSG      = LS_DBG_MSG
      EXCEPTIONS
        OTHERS       = 9.
  ...
  LOOP AT IT_LOT.
    ...
    IF SY-SUBRC EQ 0.
      LOOP AT IT_MIC1 WHERE VBELN = IT_LOT-VBELN AND POSNR = IT_LOT-POSNR AND CHARG = IT_LOT-CHARG.
        " Fix (permintaan user 16/07/2026): re-resolve MERKNR FRESH langsung dari QAMV berdasarkan
        " MIC dan lot baris ini sendiri (bukan mengandalkan MERKNR tersimpan dari GET_MIC), supaya
        " value TIDAK PERNAH "menempel"/tertukar dari MIC lain. Terbukti empiris via cek QAMR
        " langsung: tiap MIC (Elongation MD/TD, Tensile MD/TD, Haze, Gloss, Surface Tension) punya
        " MERKNR & MITTELWERT sendiri yang berbeda-beda di database - tapi ALV menampilkan value
        " yang sama persis untuk beberapa MIC berbeda (nilai milik Elongation MD ikut ke MIC lain).
        " Root cause pastinya di resolusi MERKNR upstream belum 100% terpetakan, jadi fix ini
        " langsung re-query MERKNR per baris sebagai jaminan agar value selalu sesuai MIC-nya sendiri.
        IF IT_MIC1-INSLOT IS NOT INITIAL.
          CLEAR IT_MIC1-MERKNR.
          SELECT SINGLE MERKNR INTO IT_MIC1-MERKNR
            FROM QAMV
            WHERE PRUEFLOS = IT_MIC1-INSLOT
              AND VERWMERKM = IT_MIC1-MIC.
        ENDIF.

        " Fix (permintaan user 15/07/2026): SELECT SINGLE...INTO TIDAK clear target field kalau
        " SY-SUBRC <> 0 - tanpa CLEAR ini, nilai CMICMIT dari MIC sebelumnya yang berhasil bisa
        " "bocor" ke MIC lain yang gagal ketemu di QAMR, sehingga fallback (QASE/MITTELWERT/'-')
        " ikut ke-skip karena CMICMIT dianggap sudah terisi (padahal itu nilai basi).
        CLEAR: IT_MIC1-CMICMIT, IT_MIC1-VORGLFNR, IT_MIC1-MICMIT.
  ...
  " TEMP DEBUG (permintaan user 17/07/2026): simpan log ke DB supaya bisa dilihat via SLG1
  " (Object = ZQMICOA, Subobject = TRACE). Murni instrumentasi, tidak mempengaruhi output ALV.
  IF LV_DBG_LOG_HANDLE IS NOT INITIAL.
    REFRESH LT_DBG_LOG_HANDLE.
    APPEND LV_DBG_LOG_HANDLE TO LT_DBG_LOG_HANDLE.
    CALL FUNCTION 'BAL_DB_SAVE'
      EXPORTING
        I_T_LOG_HANDLE = LT_DBG_LOG_HANDLE
      EXCEPTIONS
        OTHERS         = 9.
  ENDIF.
ENDFORM.                    "get_data
  ...
  IF LOT_SR_CONV IS INITIAL.
    ...
    IF L_JR_CONV_BATCH IS NOT INITIAL.
      ...
      LOOP AT LT_SR.
        ...
        SELECT SINGLE PRUEFLOS INTO GS_TRACE_DATA-LOT_SR_CONV_SIBLING FROM QALS WHERE CHARG = LV_LATEST_SR AND ART = 'Z04'.
        IF GS_TRACE_DATA-LOT_SR_CONV_SIBLING IS NOT INITIAL.
          LOT_SR_CONV = GS_TRACE_DATA-LOT_SR_CONV_SIBLING.
          EXIT. " Ditemukan inspection lot dari salah satu saudara SR
        ENDIF.
      ENDLOOP.
    ENDIF.
  ELSE.
...
* dibaca oleh GET_VALID_COMPONENT_FROM_MSEG (kedua form ini SELALU dipanggil berpasangan
* dan berurutan). BKTXT menyimpan identitas transaksi / nomor roll, dan merupakan SATU-SATUNYA
* penghubung yang benar antara posting 101 dan 261 dalam 1 production order.
...
FORM GET_VALID_ORDER_FROM_MSEG USING P_CHARG CHANGING P_AUFNR.
  ...
  IF SY-SUBRC = 0.
    ...
    IF SY-SUBRC = 0.
      ...
      " Fix A (permintaan user 18/07/2026): ambil BKTXT dokumen GR (101) ini, dipakai untuk
      " memilih komponen 261 yang BENAR-BENAR pasangan transaksi/roll ini. Terbukti dari data:
      " order 100000072887 punya 8 posting 261 dengan 4 batch komponen berbeda, dibedakan
      " HANYA oleh BKTXT (mis. "F GID 076 001".."004"). Logic lama ambil MBLNR terbesar ->
      " salah roll tanpa gejala. Referensi: ZQMR_TRACEABILITY (variabel VBKTXT).
      SELECT SINGLE BKTXT INTO GV_TRACE_BKTXT
        FROM MKPF
        WHERE MBLNR = LT_MSEG-MBLNR
          AND MJAHR = LT_MSEG-MJAHR.

      " Fix B (permintaan user 18/07/2026): resolve COLLECTIVE / COMBINED ORDER. AUFNR hasil
      " MSEG 101 bisa berupa order referensi (mis. A29000002540) yang TIDAK punya posting
      " komponen 261 sama sekali; order aslinya (mis. 100000072918) dicari via
      " AFPO-MILL_OC_AUFNR_U. Tanpa langkah ini tracing MATI di hop pertama sehingga MIC
      " level Base Film (mis. PALZTH00) selalu kosong. Referensi: ZQMR_TRACEABILITY (VCOMBINE).
      CLEAR LV_COMBINE.
...
FORM GET_VALID_COMPONENT_FROM_MSEG USING P_AUFNR CHANGING P_CHARG.
  DATA: BEGIN OF LT_COMP OCCURS 0,
        ...
        END OF LT_COMP.
  DATA: LV_REV_MBLNR LIKE MSEG-MBLNR.
  ...
  " Fix (permintaan user 18/07/2026): baca komponen dari AUFM (bukan MSEG) + JOIN MKPF supaya
  " dapat BKTXT, dan tambahkan movement type custom '901' selain '261'. Mengikuti pola
  " ZQMR_TRACEABILITY yang sudah terbukti benar untuk relasi 101 <-> 261.
  SELECT AUFM~AUFNR AUFM~MBLNR AUFM~MJAHR AUFM~ZEILE AUFM~BWART
         AUFM~CHARG AUFM~MATNR MKPF~BKTXT
    INTO CORRESPONDING FIELDS OF TABLE LT_COMP
    FROM AUFM INNER JOIN MKPF
      ON AUFM~MBLNR = MKPF~MBLNR
     AND AUFM~MJAHR = MKPF~MJAHR
    WHERE AUFM~AUFNR = P_AUFNR
      AND AUFM~BWART IN ('261', '901')
      AND AUFM~CHARG <> ''.
  ...
  " Filter BKTXT: ambil HANYA komponen milik transaksi/roll yang sama dengan dokumen 101-nya.
  " Kalau BKTXT dokumen 101 kosong (data lama sebelum konvensi ini dipakai), filter dilewati
  " supaya kandidat tidak habis terhapus - perilaku jatuh balik ke logic lama.
  IF GV_TRACE_BKTXT IS NOT INITIAL.
    DELETE LT_COMP WHERE BKTXT <> GV_TRACE_BKTXT.
  ENDIF.

  " Buang komponen yang sudah dibatalkan (262/902 yang me-reverse baris ini). AUFM TIDAK punya
  " field SMBLN/SJAHR/SMBLP (sudah diverifikasi via SE11), jadi cek pembatalan dilakukan ke MSEG.
  LOOP AT LT_COMP.
    CLEAR LV_REV_MBLNR.
    SELECT SINGLE MBLNR INTO LV_REV_MBLNR
      FROM MSEG
      WHERE MATNR = LT_COMP-MATNR
        AND CHARG = LT_COMP-CHARG
        AND BWART IN ('262', '902')
        AND SMBLN = LT_COMP-MBLNR
        AND SJAHR = LT_COMP-MJAHR
        AND SMBLP = LT_COMP-ZEILE.
    IF SY-SUBRC = 0 AND LV_REV_MBLNR IS NOT INITIAL.
      DELETE LT_COMP.
    ENDIF.
  ENDLOOP.

  SORT LT_COMP BY MBLNR DESCENDING ZEILE DESCENDING.
  LOOP AT LT_COMP.
    " Pastikan yang diambil adalah material Base Film (SR/JR), bukan komponen pelengkap
    IF LT_COMP-MATNR(2) = 'SR' OR LT_COMP-MATNR(2) = 'JR'.
      P_CHARG = LT_COMP-CHARG.
      EXIT.
    ENDIF.
  ENDLOOP.
ENDFORM.
