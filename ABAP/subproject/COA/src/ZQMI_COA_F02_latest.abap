  ...
  CASE SY-UCOMM.
    ...
    WHEN 'EXEC'.
      ...
      IF SY-SUBRC = 0.
        ...
        ENDLOOP.
      ENDIF.
      CLEAR:CSTAT, TBATCH, IT_DETAIL, T_DETAIL1, IT_DETAIL1, T_DETAIL,IT_WIDTH,WIDTH
...
MODULE TDETAIL_USER_COMMAND INPUT.
  ...
  CASE SY-UCOMM.
    ...
    WHEN 'PRNT'.
      ...
      LOOP AT IT_DETAIL WHERE CMICMIT EQ '-' OR CMICMIT IS INITIAL OR CMICMIT EQ '00
        LV_EMPTY_FOUND = 'X'.
        EXIT.
      ENDLOOP.
        ...
        LOOP AT IT_DETAIL WHERE TYPE EQ 'X'.
          ...
          TRY.
            ...
            CATCH CX_ROOT.
              CONTINUE.
          ENDTRY.
          IF ( IT_DETAIL-RAW_MICMIN IS NOT INITIAL AND LV_CHK_VAL < IT_DETAIL-RAW_MI
             ( IT_DETAIL-RAW_MICMAX IS NOT INITIAL AND LV_CHK_VAL > IT_DETAIL-RAW_MI
            LV_RANGE_FOUND = 'X'.
            EXIT.
          ENDIF.
        ENDLOOP.
      ENDIF.
      IF LV_EMPTY_FOUND = 'X'.
        MESSAGE 'Ada data MIC yang belum di-record (kosong), tidak bisa cetak COA!'
      ELSEIF LV_RANGE_FOUND = 'X'.
        MESSAGE 'Ada value MIC yang keluar dari Lower/Upper Limit. Ubah value & Save
      ELSE.
        PERFORM F_PRINT.
      ENDIF.
  ENDCASE.
  ...
  IF LV_HAS_CHANGES = 'X'.
    CALL FUNCTION 'POPUP_TO_CONFIRM'
      EXPORTING
        ...
        TEXT_QUESTION         = 'Ada perubahan nilai MIC. Apakah Anda yakin ingin me
...
FORM CANCEL_UD USING PRUEFLOS CHANGING LS_SUBRC MESS ZSTAT ERR.
  ...
  IF SY-SUBRC = 0.
    SELECT SINGLE TEXT INTO MESS FROM T100 WHERE SPRSL = 'E' AND ARBGB = MESSTAB-MSG
...
FORM SET_INDICATOR USING INDEX COUNT_INDEX PROCESS COUNT_PROCESS TEXT.
  DATA: PERCENT TYPE P.
  CLEAR: PERCENT.
  PERCENT =  ( ( INDEX + ( COUNT_INDEX * PROCESS ) - COUNT_INDEX ) / ( COUNT_INDEX *
