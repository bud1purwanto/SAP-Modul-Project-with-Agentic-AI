...
FORM GET_COA_COL_VALUE USING P_FIELD_NAME TYPE ANY
  ...
  CASE P_FIELD_NAME.
    ...
    WHEN 'PO_NUMBER' OR 'PO_NUM'.
      SELECT SINGLE BSTKD INTO P_RAW_VAL FROM VBKD WHERE VBELN = P_BATCH-VBELN.
    WHEN 'THICK'.
      P_RAW_VAL = ''.
    WHEN 'NO_PALET' OR 'PALET' OR 'HU_NUMBER' OR 'HU_NUM'.
      DATA: LV_HPAL TYPE C LENGTH 20,
    ...
    WHEN 'TYPE' OR 'MAT_TYPE' OR 'ZZTYPE' OR 'ZZCODE'.
      DATA: LV_ZZCODE_VAL TYPE AUSP-ATWRT,
            LV_CUOBJ_CD   TYPE MCH1-CUOBJ_BM,
            LV_ATINN_CD   TYPE CABN-ATINN,
            LV_ATZHL_CD   TYPE CAWN-ATZHL,
            LV_DESC_CD    TYPE CAWNT-ATWTB.
      CLEAR: LV_ZZCODE_VAL, LV_CUOBJ_CD, LV_ATINN_CD, LV_ATZHL_CD, LV_DESC_CD.
      ...
      IF LV_CUOBJ_CD IS INITIAL.
        SELECT SINGLE CUOBJ_BM INTO LV_CUOBJ_CD FROM MCHA
          WHERE CHARG = P_BATCH-CHARG.
      ENDIF.
      IF LV_CUOBJ_CD IS NOT INITIAL.
        CALL FUNCTION 'CONVERSION_EXIT_ATINN_INPUT'
          EXPORTING INPUT  = 'ZZCODE'
          IMPORTING OUTPUT = LV_ATINN_CD.
        IF LV_ATINN_CD IS NOT INITIAL.
          SELECT SINGLE ATWRT INTO LV_ZZCODE_VAL FROM AUSP
            WHERE OBJEK = LV_CUOBJ_CD AND ATINN = LV_ATINN_CD.
        ENDIF.
      ENDIF.

      IF LV_ZZCODE_VAL IS NOT INITIAL.
        SELECT SINGLE ATZHL INTO LV_ATZHL_CD FROM CAWN
          WHERE ATINN = LV_ATINN_CD AND ATWRT = LV_ZZCODE_VAL.
        IF LV_ATZHL_CD IS NOT INITIAL.
          SELECT SINGLE ATWTB INTO LV_DESC_CD FROM CAWNT
            WHERE ATINN = LV_ATINN_CD AND ATZHL = LV_ATZHL_CD AND SPRAS = SY-LANGU.
          IF LV_DESC_CD IS INITIAL.
            SELECT SINGLE ATWTB INTO LV_DESC_CD FROM CAWNT
              WHERE ATINN = LV_ATINN_CD AND ATZHL = LV_ATZHL_CD.
          ENDIF.
        ENDIF.
      ENDIF.
    ...
    WHEN 'WEIGHT_PER_ROL' OR 'WEIGHT_ROL'.
      DATA: LV_NTGEW_V TYPE LIPS-NTGEW.
    ...
    WHEN 'TOTAL_ROLL' OR 'TOT_ROLL'.
      DATA: LV_HPAL2 TYPE C LENGTH 20,
            LV_HROL2 TYPE C LENGTH 10,
            LV_HWGT2 TYPE C LENGTH 20.
      CLEAR: LV_HPAL2, LV_HROL2, LV_HWGT2.
      PERFORM GET_HU_PALLET_INFO USING P_BATCH-VBELN P_BATCH-CHARG
                              CHANGING LV_HPAL2 LV_HROL2 LV_HWGT2.
      P_RAW_VAL = LV_HROL2.
    WHEN 'TOTAL_WEIGHT_PALET' OR 'TOT_PAL_WGT' OR 'TOTAL_WGT_PALET'.
      DATA: LV_HPAL3 TYPE C LENGTH 20,
            LV_HROL3 TYPE C LENGTH 10,
            LV_HWGT3 TYPE C LENGTH 20.
      CLEAR: LV_HPAL3, LV_HROL3, LV_HWGT3.
      PERFORM GET_HU_PALLET_INFO USING P_BATCH-VBELN P_BATCH-CHARG
                              CHANGING LV_HPAL3 LV_HROL3 LV_HWGT3.
      P_RAW_VAL = LV_HWGT3.
    WHEN 'GG_PART_NUMBER' OR 'GG_PART_NO' OR 'NO_PALET_TRIAS' OR 'PALET_TRIAS' OR
         'LENGTH_OF_SPLICE' OR 'SPLICE_LEN'.
      P_RAW_VAL = ''.
    WHEN 'EXPIRED_DATE' OR 'EXP_DATE' OR 'USED_BEFORE'.
      SELECT SINGLE VFDAT INTO P_RAW_VAL FROM MCH1 WHERE CHARG = P_BATCH-CHARG.
    WHEN 'PRODUCTION_DATE' OR 'PROD_DATE' OR 'MANUFACTURING_DATE' OR 'MFG_DATE'.
      SELECT SINGLE HSDAT INTO P_RAW_VAL FROM MCH1 WHERE CHARG = P_BATCH-CHARG.
    WHEN 'NUMBER_OF_JOINT' OR 'SPLICE_QTY'.
      P_RAW_VAL = '0'.
    WHEN 'LOT_NUMBER' OR 'LOT_NUM'.
      P_RAW_VAL = P_BATCH-INSLOT.
    WHEN 'SO_NUMBER' OR 'SO_NUM'.
      SELECT SINGLE VBELN INTO P_RAW_VAL FROM VBFA
        WHERE VBELV = P_BATCH-VBELN AND VBTYP_N = 'C'.
