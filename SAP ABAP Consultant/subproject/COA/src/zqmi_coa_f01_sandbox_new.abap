...
FORM GET_TRACED_LOTS.
  ...
  " Fallback jika terkena Change Grade ZQM0016 atau Kupas ZPP019
  " Cek setiap history satu per satu agar tidak melompati SR yang diinspeksi
  IF LOT_SR_CONV IS INITIAL.
    LV_SEARCH_SR = IT_DATA-CHARG.
    DO.
      PERFORM GET_PREVIOUS_BATCH USING LV_SEARCH_SR CHANGING LV_PREV_SR.
      IF LV_PREV_SR IS INITIAL OR LV_PREV_SR = LV_SEARCH_SR.
        EXIT.
      ENDIF.
      LV_SEARCH_SR = LV_PREV_SR.
...
*&---------------------------------------------------------------------*
*&      Form  GET_PREVIOUS_BATCH
*&---------------------------------------------------------------------*
FORM GET_PREVIOUS_BATCH USING P_BATCH CHANGING P_PREV_BATCH.
    CLEAR P_PREV_BATCH.
    SELECT CHARG INTO P_PREV_BATCH FROM ZBATCHISTORY
      UP TO 1 ROWS
      WHERE NCHARG = P_BATCH
      ORDER BY BUDAT DESCENDING UZEIT DESCENDING.
    ENDSELECT.
ENDFORM.
