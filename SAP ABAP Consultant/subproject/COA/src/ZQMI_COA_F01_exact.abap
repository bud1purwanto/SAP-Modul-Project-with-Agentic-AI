...
START-OF-SELECTION.
  ...
  ELSEIF P_VBELN IS INITIAL.
    MESSAGE 'Nomor ODO (Delivery) WAJIB diisi!' TYPE 'I'.
    STOP.
  ELSE.
    PERFORM GET_DATA.
...
FORM GET_DATA.
  ...
  LOOP AT IT_DATA.
    ...
    IF L_SUBRC EQ 0.
      ...
      IF IT_ZMAP[] IS INITIAL.
        MESSAGE 'Material belum dimapping di ZMAP_COA!' TYPE 'I'.
        STOP.
      ENDIF.
    ENDIF.
    " Edited by J. Budi (Antigravity) on 28.06.2026
  ...
  IT_DATA[] = IT_DATA_AVG[].
  LOOP AT IT_DATA.
    IF IT_DATA-CMICMIT IS INITIAL OR IT_DATA-CMICMIT EQ '-' OR IT_DATA-CMICMIT EQ '0'.
      IT_DATA-CMICMIT = '0'.
    ENDIF.
...
FORM F_PRINT .
  ...
  ENDIF.
  CLEAR LV_BATCH_FM.
  IF GV_TIER = 'LANDSCAPE' AND GT_DYN_ROLL[] IS NOT INITIAL.
    CALL FUNCTION 'SSF_FUNCTION_MODULE_NAME'
      EXPORTING
  ...
  IF SY-SUBRC <> 0.
    ...
    EXIT.
  ENDIF.
  IF GV_TIER = 'LANDSCAPE' AND LV_BATCH_FM IS NOT INITIAL AND GT_DYN_ROLL[] IS NOT INITIAL.
