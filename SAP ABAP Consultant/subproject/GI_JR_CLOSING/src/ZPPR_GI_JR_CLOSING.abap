...
DATA: GT_GR       TYPE STANDARD TABLE OF TY_MOV,
      ...
      GT_REL      TYPE SORTED TABLE OF TY_REL
                  WITH UNIQUE KEY OUT_ORDER MOV_ORDER WERKS MATNR CHARG BKTXT,
      GT_AFPO     TYPE STANDARD TABLE OF TY_AFPO,
      GT_AFPO_VER TYPE SORTED TABLE OF TY_AFPO_VER
...
START-OF-SELECTION.
  ...
  IF P_MAIL = 'X'.
    IF P_TEST IS INITIAL AND P_TMAIL IS INITIAL.
...
FORM VALIDATE_SELECTION CHANGING CV_ERROR TYPE CHAR1.
  ...
  LOOP AT S_WERKS INTO LS_WERKS WHERE SIGN = 'I'.
    AUTHORITY-CHECK OBJECT 'M_MSEG_WMB'
      ID 'ACTVT' FIELD '03'
      ID 'WERKS' FIELD LS_WERKS-LOW.
    IF SY-SUBRC <> 0.
      MESSAGE 'Tidak berwenang membaca material document untuk plant.'
  ...
  IF P_MAIL = 'X' AND P_TEST = 'X' AND P_TMAIL = 'X'.
    MESSAGE 'Test Run dan Test Kirim Email tidak boleh bersamaan.'
      TYPE 'S' DISPLAY LIKE 'E'.
    CV_ERROR = 'X'.
    RETURN.
  ENDIF.
