import { mkdirSync, writeFileSync } from 'node:fs';
import { resolve } from 'node:path';

const { HANDLERS } = await import(
  'file:///var/www/MCP/MCP%20SAP/sap-leader-mcp/src/tool-registry.js'
);

const programName = 'ZPPR_GI_JR_CLOSING';

function replaceOnce(source, search, replacement, label) {
  const first = source.indexOf(search);
  if (first < 0 || source.indexOf(search, first + search.length) >= 0) {
    throw new Error(`Expected one ${label} block.`);
  }
  return source.replace(search, replacement);
}

function replaceFirst(source, search, replacement, label) {
  const first = source.indexOf(search);
  if (first < 0) {
    throw new Error(`Expected ${label} block.`);
  }
  return source.slice(0, first) + replacement + source.slice(first + search.length);
}

const server = await HANDLERS.set_active_server({ server_ref: 'sandbox-new' });
if (!server.connected || server.sid !== 'TRS') {
  throw new Error(`TRS connection unavailable: ${JSON.stringify(server)}`);
}

const live = await HANDLERS.read_program({ program_name: programName });
if (live.mode !== 'LIVE' || !live.source) {
  throw new Error('Unable to read the live TRS source.');
}

const backupDir = resolve('subproject/GI_JR_CLOSING/outputs');
mkdirSync(backupDir, { recursive: true });
writeFileSync(
  resolve(backupDir, 'ZPPR_GI_JR_CLOSING_backup_20260910_TRS_before_performance.abap'),
  `${live.source}\n`,
  'utf8'
);

let source = live.source;

source = replaceOnce(
  source,
  `      GT_GIMOV    TYPE STANDARD TABLE OF TY_MOV,\n`,
  `      GT_GIMOV    TYPE SORTED TABLE OF TY_MOV\n                  WITH NON-UNIQUE KEY AUFNR WERKS MATNR CHARG BKTXT,\n`,
  'GT_GIMOV declaration'
);

source = replaceOnce(
  source,
  `      GT_MAP_REC  TYPE STANDARD TABLE OF ZMAP_TYPE,\n`,
  `      GT_MAP_REC  TYPE SORTED TABLE OF ZMAP_TYPE\n                  WITH NON-UNIQUE KEY OPT,\n`,
  'GT_MAP_REC declaration'
);

source = replaceFirst(
  source,
  `  SELECT * INTO TABLE GT_MAP_REC FROM ZMAP_TYPE\n   WHERE PROG = SY-CPROG\n     AND TYPE = GC_MAP_MAIL\n     AND DELETION = SPACE.`,
  `  SELECT TCODE PROG TYPE OPT VALUE DELETION\n    INTO CORRESPONDING FIELDS OF TABLE GT_MAP_REC\n    FROM ZMAP_TYPE\n   WHERE PROG = SY-CPROG\n     AND TYPE = GC_MAP_MAIL\n     AND DELETION = SPACE.`,
  'ZMAP_TYPE recipient load'
);

source = replaceOnce(
  source,
  `\n  SELECT * INTO TABLE GT_MAP_REC FROM ZMAP_TYPE\n   WHERE PROG = SY-CPROG\n     AND TYPE = GC_MAP_MAIL\n     AND DELETION = SPACE.\n\n  LOOP AT GT_RESULT INTO LS_OUT.`,
  `\n  LOOP AT GT_RESULT INTO LS_OUT.`,
  'duplicate ZMAP_TYPE recipient load'
);

const requiredMkpfFilters = [
  'FROM MKPF AS B INNER JOIN MSEG AS A',
  'WHERE B~BUDAT IN S_BUDAT',
  'FROM MSEG AS A INNER JOIN MKPF AS B',
  'AND B~BUDAT LE LV_CUTOFF'
];
for (const filter of requiredMkpfFilters) {
  if (!source.includes(filter)) {
    throw new Error(`MKPF-BUDAT filter missing: ${filter}`);
  }
}

const lines = source.split('\n');
if (lines.some((line) => line.length > 255)) {
  throw new Error('Patched source contains a line longer than 255 characters.');
}

const update = await HANDLERS.call_function({
  function_name: 'Z_RFC_PROGRAM_UPDATE',
  parameters: {
    IV_PROGRAM_NAME: programName,
    IV_PACKAGE: '$TMP',
    IV_CORRNUMBER: ' ',
    IT_SOURCE: lines.map((LINE) => ({ LINE }))
  }
});

if (update.mode !== 'LIVE' || !update.result?.EV_SUCCESS) {
  throw new Error(`Source update failed: ${JSON.stringify(update)}`);
}

console.log(JSON.stringify({
  backup: 'subproject/GI_JR_CLOSING/outputs/ZPPR_GI_JR_CLOSING_backup_20260910_TRS_before_performance.abap',
  sourceLinesBefore: live.line_count,
  sourceLinesAfter: lines.length,
  update: update.result
}, null, 2));
