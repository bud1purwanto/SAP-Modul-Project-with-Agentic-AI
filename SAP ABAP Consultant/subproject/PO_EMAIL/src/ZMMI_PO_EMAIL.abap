...
* VENDOR = harga terakhir material dari vendor yang sama.
  READ TABLE LT_EXC_RULE INTO LS_EXC_RULE
    WITH KEY OPT = 'PRICE_INCREASE' VALUE = 'VENDOR'
    BINARY SEARCH.
  IF SY-SUBRC EQ 0.
    LV_RULE_PRICE_VEN = 'X'.
  ENDIF.
...
*       Masing-masing cabang hanya berjalan jika mapping aktif.
        ...
        ELSE.
          IF LV_RULE_STALE EQ 'X'
             AND LS_LASTM-BEDAT LT LV_CUTSTALE.
            LV_EXC = 'X'.
            PERFORM F_ADD_REASON USING 'StaleHist'
                                 CHANGING LS_RESULT-EXC_REASON.
          ELSEIF LV_RULE_PRICE EQ 'X' AND LV_RULE_PRICE_MAT EQ 'X'.
...
*       Bila stale rule aktif, histori vendor usang tidak dibandingkan.
        IF LV_RULE_PRICE EQ 'X' AND LV_RULE_PRICE_VEN EQ 'X'
           AND LV_HASVEN IS NOT INITIAL.
          IF LV_RULE_STALE IS INITIAL
             OR LS_LASTV-BEDAT GE LV_CUTSTALE.
...
* Kolom kunci dan exception diletakkan lebih awal untuk pembacaan cepat.
  ...
  PERFORM F_ADD_FIELDCAT USING 'EKGRP' 'Purch. Group' 11 SPACE.
...
FORM F_OUTPUT_LIST.
  ...
  SKIP.
  WRITE: / SY-ULINE.
  WRITE: / 'PO Number', 15 'Purch. Grp', 29 'USD Val', 49 'Tier',
           56 'Approver', 74 'Email', 98 'St', 102 'Ex',
           106 'Alasan Exception', 169 'Keterangan'.
  WRITE: / SY-ULINE.
  ...
  LOOP AT LT_RESULT INTO LS_RESULT.
    WRITE: / LS_RESULT-EBELN,
             ...
             29 LS_RESULT-USDVAL,
             49 LS_RESULT-TIER,
             56 LS_RESULT-APPR_NAME,
             74 LS_RESULT-APPR_MAIL,
             98 LS_RESULT-STATUS,
             102 LS_RESULT-EXC_FLAG,
             106(60) LS_RESULT-EXC_REASON,
             169 LS_RESULT-MESSAGE.
  ENDLOOP.
